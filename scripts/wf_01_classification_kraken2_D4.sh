...
# Crete the Out Dir
mkdir -p $DATAPATH_CLASSIFICATION_KRAKEN2_D4_OUT
...
for fq in $DATAPATH_CLASSIFICATION_KRAKEN2_D4_IN*.fastq
  do
    $PATH_KRAKEN2 --report $DATAPATH_CLASSIFICATION_KRAKEN2_D4_OUT$(basename $fq)_REPORT --db $PATH_DB $fq > $DATAPATH_CLASSIFICATION_KRAKEN2_D4_OUT$(basename $fq)_CLASSIFICATION
  done